...
class GroupPointerNet(select_module.SelectNet):
    ...
    def forward(self, q_data, d_data, q_len, d_len, q_id, d_id):
        ...
        for i in range(len(q_len)):
            for j in range(q_len[i].item()):
                embed_q_list.append(embed_q[i])
                embed_d_list.append(embed_d[i])
                embed_q_list[-1][j] += 0.5
                q_len_list.append(q_len[i])

        embed_q = torch.stack(embed_q_list, dim=0)
        embed_d = torch.stack(embed_d_list, dim=0)
        q_len_new = torch.stack(q_len_list, dim=0)
        ...
        # B x E
        vec_q = torch.sum(embed_q_r, dim=2, keepdim=False) / q_len_new.unsqueeze(dim=1).type(torch.float32)
        ...
        for i in range(len(q_len)):
            ...
            snippet_val = []
            for j in range(q_len[i].item()):
                snippet.append(self.process_item(d_data[i], self.top_k_idx[k]))
                k += 1
            snippets.append(torch.cat(snippet, dim=0))
        
        positions = self.top_k_idx.split(q_len.tolist())
        positions_val = self.top_k_val.split(q_len.tolist())
        #positions = torch.unbind(self.top_k_idx)
